"""Minimal dependency-free bcrypt verifier for imported Cognito credentials.

This module deliberately implements verification only. It accepts the fixed
OpenBSD variants used by Cognito imports, caps the logarithmic work factor, and
never invokes native processes or generates new password hashes.
"""
...
_MASK = 0xFFFFFFFF
_CIPHERTEXT = b"OrpheanBeholderScryDoubt"
_BCRYPT64 = b"./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
_BCRYPT64_INDEX = {value: index for index, value in enumerate(_BCRYPT64)}
_HASH = re.compile(rb"^\$(?P<version>2[aby])\$(?P<cost>[0-9]{2})\$(?P<body>[./A-Za-z0-9]{53})$")

# First 8,336 hexadecimal fractional digits of pi, the standard Blowfish P/S
...
_INITIAL_WORDS = struct.unpack(">1042I", bytes.fromhex(_INITIAL_HEX))


@dataclass(frozen=True)
class BcryptWorkBudget:
    minimum_cost: int = 4
    maximum_cost: int = 10
    maximum_password_bytes: int = 72
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or not 4 <= self.minimum_cost <= self.maximum_cost <= 31
            or self.maximum_password_bytes != 72
        ):
            raise ValueError("invalid bcrypt work budget")


DEFAULT_BCRYPT_BUDGET = BcryptWorkBudget()


class BcryptBudgetExceeded(RuntimeError):
    """The cooperative wall or CPU deadline expired during key setup."""
...
def verify_bcrypt(
    ...
    encoded: object,
    *,
    budget: BcryptWorkBudget = DEFAULT_BCRYPT_BUDGET,
) -> bool:
    """Verify a bounded modular bcrypt hash, failing closed on invalid input."""
    try:
        return verify_bcrypt_bounded(password, encoded, budget=budget)
    except BcryptBudgetExceeded:
        return False


def verify_bcrypt_bounded(
    password: object,
    encoded: object,
    *,
    budget: BcryptWorkBudget = DEFAULT_BCRYPT_BUDGET,
    ...
    """Verify bcrypt and raise when a cooperative execution deadline expires."""
    try:
        check_budget = _execution_budget(maximum_wall_seconds, maximum_cpu_seconds)
        if not isinstance(password, (str, bytes)) or not isinstance(encoded, (str, bytes)):
            return False
        password_bytes = password.encode("utf-8") if isinstance(password, str) else password
        encoded_bytes = encoded.encode("ascii") if isinstance(encoded, str) else encoded
        if len(encoded_bytes) != 60 or len(password_bytes) > 131_072:
            return False
        match = _HASH.fullmatch(encoded_bytes)
        if match is None:
            return False
        cost = int(match.group("cost"))
        if not budget.minimum_cost <= cost <= budget.maximum_cost:
            return False
        salt_text, expected_digest = match.group("body")[:22], match.group("body")[22:]
        salt = _bcrypt64_decode(salt_text, 16)
        if _bcrypt64_encode(salt) != salt_text:
            return False
        candidate = _bcrypt_raw(
            password_bytes[: budget.maximum_password_bytes], salt, cost, check_budget
        )
        return hmac.compare_digest(_bcrypt64_encode(candidate[:23]), expected_digest)
    except BcryptBudgetExceeded:
        raise
    except Exception:
        return False
...
def _bcrypt_raw(
    ...
    key = password + b"\0"
    words = list(_INITIAL_WORDS)
    p = words[:18]
    s = words[18:]
    if check_budget is not None:
        check_budget()
    _expand_key(p, s, salt, key)
    for round_index in range(1 << cost):
        if check_budget is not None and round_index & 3 == 0:
            check_budget()
        _expand_zero(p, s, key)
        _expand_zero(p, s, salt)
    ciphertext = list(struct.unpack(">6I", _CIPHERTEXT))
    for round_index in range(64):
        if check_budget is not None and round_index & 7 == 0:
            check_budget()
        for offset in range(0, 6, 2):
            ciphertext[offset], ciphertext[offset + 1] = _encipher(
                ciphertext[offset], ciphertext[offset + 1], p, s
            )
    return struct.pack(">6I", *ciphertext)


def _execution_budget(
    maximum_wall_seconds: Any,
    maximum_cpu_seconds: Any,
):
    if maximum_wall_seconds is None and maximum_cpu_seconds is None:
        return None
    for value in (maximum_wall_seconds, maximum_cpu_seconds):
        if value is not None and (
            ...
            raise ValueError("invalid bcrypt execution budget")
    wall_started = time.monotonic()
    cpu_started = time.thread_time()

    def check() -> None:
        if maximum_wall_seconds is not None and time.monotonic() - wall_started >= float(
            maximum_wall_seconds
        ):
            raise BcryptBudgetExceeded("bcrypt wall budget exceeded")
        if maximum_cpu_seconds is not None and time.thread_time() - cpu_started >= float(
            maximum_cpu_seconds
        ):
            raise BcryptBudgetExceeded("bcrypt CPU budget exceeded")

    return check


def _expand_key(p: list[int], s: list[int], salt: bytes, key: bytes) -> None:
    key_offset = 0
    for index in range(18):
        word, key_offset = _stream_word(key, key_offset)
        p[index] ^= word
    salt_offset = 0
    left = right = 0
    for index in range(0, 18, 2):
        word, salt_offset = _stream_word(salt, salt_offset)
        left ^= word
        word, salt_offset = _stream_word(salt, salt_offset)
        right ^= word
        left, right = _encipher(left, right, p, s)
        p[index], p[index + 1] = left, right
    for index in range(0, 1024, 2):
        ...
        right ^= word
        left, right = _encipher(left, right, p, s)
        s[index], s[index + 1] = left, right


def _expand_zero(p: list[int], s: list[int], key: bytes) -> None:
    key_offset = 0
    for index in range(18):
        word, key_offset = _stream_word(key, key_offset)
        p[index] ^= word
...
def _stream_word(data: bytes, offset: int) -> tuple[int, int]:
    word = 0
    length = len(data)
    for _ in range(4):
...
def _encipher(left: int, right: int, p: list[int], s: list[int]) -> tuple[int, int]:
    left ^= p[0]
    mixed = (s[left >> 24] + s[256 + ((left >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((left >> 8) & 0xFF)]
    right ^= ((mixed + s[768 + (left & 0xFF)]) & _MASK) ^ p[1]
    mixed = (s[right >> 24] + s[256 + ((right >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((right >> 8) & 0xFF)]
    left ^= ((mixed + s[768 + (right & 0xFF)]) & _MASK) ^ p[2]
    mixed = (s[left >> 24] + s[256 + ((left >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((left >> 8) & 0xFF)]
    right ^= ((mixed + s[768 + (left & 0xFF)]) & _MASK) ^ p[3]
    mixed = (s[right >> 24] + s[256 + ((right >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((right >> 8) & 0xFF)]
    left ^= ((mixed + s[768 + (right & 0xFF)]) & _MASK) ^ p[4]
    mixed = (s[left >> 24] + s[256 + ((left >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((left >> 8) & 0xFF)]
    right ^= ((mixed + s[768 + (left & 0xFF)]) & _MASK) ^ p[5]
    mixed = (s[right >> 24] + s[256 + ((right >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((right >> 8) & 0xFF)]
    left ^= ((mixed + s[768 + (right & 0xFF)]) & _MASK) ^ p[6]
    mixed = (s[left >> 24] + s[256 + ((left >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((left >> 8) & 0xFF)]
    right ^= ((mixed + s[768 + (left & 0xFF)]) & _MASK) ^ p[7]
    mixed = (s[right >> 24] + s[256 + ((right >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((right >> 8) & 0xFF)]
    left ^= ((mixed + s[768 + (right & 0xFF)]) & _MASK) ^ p[8]
    mixed = (s[left >> 24] + s[256 + ((left >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((left >> 8) & 0xFF)]
    right ^= ((mixed + s[768 + (left & 0xFF)]) & _MASK) ^ p[9]
    mixed = (s[right >> 24] + s[256 + ((right >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((right >> 8) & 0xFF)]
    left ^= ((mixed + s[768 + (right & 0xFF)]) & _MASK) ^ p[10]
    mixed = (s[left >> 24] + s[256 + ((left >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((left >> 8) & 0xFF)]
    right ^= ((mixed + s[768 + (left & 0xFF)]) & _MASK) ^ p[11]
    mixed = (s[right >> 24] + s[256 + ((right >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((right >> 8) & 0xFF)]
    left ^= ((mixed + s[768 + (right & 0xFF)]) & _MASK) ^ p[12]
    mixed = (s[left >> 24] + s[256 + ((left >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((left >> 8) & 0xFF)]
    right ^= ((mixed + s[768 + (left & 0xFF)]) & _MASK) ^ p[13]
    mixed = (s[right >> 24] + s[256 + ((right >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((right >> 8) & 0xFF)]
    left ^= ((mixed + s[768 + (right & 0xFF)]) & _MASK) ^ p[14]
    mixed = (s[left >> 24] + s[256 + ((left >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((left >> 8) & 0xFF)]
    right ^= ((mixed + s[768 + (left & 0xFF)]) & _MASK) ^ p[15]
    mixed = (s[right >> 24] + s[256 + ((right >> 16) & 0xFF)]) & _MASK
    mixed ^= s[512 + ((right >> 8) & 0xFF)]
    left ^= ((mixed + s[768 + (right & 0xFF)]) & _MASK) ^ p[16]
    return (right ^ p[17]) & _MASK, left & _MASK


def _bcrypt64_encode(data: bytes) -> bytes:
    result = bytearray()
    offset = 0
    while offset < len(data):
        first = data[offset]
        offset += 1
        result.append(_BCRYPT64[first >> 2])
        first = (first & 0x03) << 4
        if offset >= len(data):
            result.append(_BCRYPT64[first])
            break
        second = data[offset]
        offset += 1
        first |= second >> 4
        ...
        first |= second >> 6
        result.append(_BCRYPT64[first])
        result.append(_BCRYPT64[second & 0x3F])
    return bytes(result)


def _bcrypt64_decode(data: bytes, expected_bytes: int) -> bytes:
    accumulator = 0
    bits = 0
    result = bytearray()
    for value in data:
        index = _BCRYPT64_INDEX.get(value)
        if index is None:
            raise ValueError("invalid bcrypt base64")
    ...
    if len(result) != expected_bytes:
        raise ValueError("invalid bcrypt base64 length")
    return bytes(result)
