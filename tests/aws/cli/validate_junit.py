...
DEFAULT_SCENARIO = "bootstrap-show-template-v32"
EXPECTED_TESTS = {
    DEFAULT_SCENARIO: (
        "tests.aws.cli.test_cdk_cli_blackbox",
        "test_cdk_cli_bootstrap_show_template_matches_pinned_v32",
    ),
...
def validate_junit_payload(payload: bytes, *, scenario: str = DEFAULT_SCENARIO) -> None:
    try:
        expected_classname, expected_test_name = EXPECTED_TESTS[scenario]
    except KeyError as error:
        raise ValueError(f"unknown JUnit scenario: {scenario}") from error
...
def validate_junit(path: Path, *, scenario: str = DEFAULT_SCENARIO) -> None:
    validate_junit_payload(load_junit(path), scenario=scenario)


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--scenario", choices=tuple(EXPECTED_TESTS), default=DEFAULT_SCENARIO)
    parser.add_argument("reports", type=Path, nargs="+")
    args = parser.parse_args()
    for report in args.reports:
        validate_junit(report, scenario=args.scenario)
    return 0
